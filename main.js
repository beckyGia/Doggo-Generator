...
fetch(BREEDS_URL)
    ...
    .then(data => {
      ...
      for (let i = 0; i < breedsArray.length; i++) {
        ...
        option.value = breedsArray[i]; //<option value= breed[i]></option>
        option.innerText = breedsArray[i]; ////<option value= breed[i]>breed[i]</option>
        select.appendChild(option); //adds current <option> tag to the select box list of options
          //creating a list of options to be used in our select tag (dropdown list)
          //appendChild is adding a child element to the element we used (the select which is select element or dropdown element).  Basically attach another tag to the parent tag
      }
    })
...
select.addEventListener('change', event => {
  ...
  doggoInfo.assignAddInfo()
})

//selecting the img tag so we can replace the image
const img = document.querySelector('.dog-img');

//here we are declaring the getDoggoImg function using an arrow fxn
...
const doggoInfo = {
  ...
  assignMF() {
    //creating a randon number that will be floored so that it is either 0 or 1
    let x = (Math.floor(Math.random() * 2 ) == 0)
    //its essentially saying if (x is true) then female else male.
    if (x) {
      this.MF = "Female";
  ...
  //best way to randomly shuffle an array
  //FISHER-YATES SHUFFLE - A simple but effective way of pull a random WORD/SENTENCE/CARD from the deck/PILE/ARRAY repeatedly and then set it aside, incrementally building a new stack.
  yatesShuffle(array) {
    //m is array.length and also declaring t and i
    let m = array.length, t, i;
  //while there remain elements to shuffle...
    while (m) {
      //Pick a remaining element (an element that hasnt been touched yet)..
